import {
  ...
  Drawer,
  List,
  // ListItem, // Removed as it's no longer used here
  ListItemText,
  Typography,
  Button,
  Divider,
  ListItemButton, // Keep this one, it's now used
} from "@mui/material";
import AddIcon from "@mui/icons-material/Add";
import { ChatMessage } from "./types/types";
import { format } from "date-fns";
// Note: Removed duplicate ListItemButton import if it existed
          ...
          <Box key={date} sx={{ mb: 2 }}>
            ...
            <List>
              {groupedChats[date].map((chat) => (
                // --- CHANGE HERE ---
                // Replaced ListItem with ListItemButton and removed the 'button' prop
                <ListItemButton key={chat.id} onClick={() => loadChat(chat.id)}>
